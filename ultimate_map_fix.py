...
# Read the full detailed_world.svg
svg_content = open('detailed_world.svg', 'r', encoding='utf-8').read()
paths = re.findall(r'<path[^>]*d="([^"]+)"', svg_content)

new_svg = f'''<svg class="svg-draw" viewBox="0 0 4378 2434" fill="rgba(226,49,55,0.05)" stroke="var(--color-brand-red)" stroke-width="2" stroke-linejoin="round" stroke-linecap="round" style="width: 100%; height: auto; filter: drop-shadow(0 10px 15px rgba(226,49,55,0.15));" xmlns="http://www.w3.org/2000/svg">'''
...
new_svg += '''
    ...
    <!-- Positioned for 4378x2434 viewBox -->
...
print("Successfully applied the exact user pasted HTML and map!")
