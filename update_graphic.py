...
# Replace the entire old svg with the new one
start_idx = content.find('<svg class="philosophy-draw"')
end_idx = content.find('</svg>', start_idx) + 6

final_content = content[:start_idx] + new_svg + content[end_idx:]

open("index.html", "w", encoding="utf-8").write(final_content)
print("Updated SVG graphic!")
